module DropboxSync
  def self.update_photo(item, type, force=false)
  ...
  def self.update_with_cursor(dbx, cursor, entity, allowed_type=Dropbox::FolderMetadata, **kwargs)
  ...
  def self.update_all(dbx, path, elements, entity, allowed_type=Dropbox::FolderMetadata, **kwargs)
  ...
  def self.match_generations(dbx, item, entity)
    changes = []
    if item.cursor
  ...
  def self.match_group(dbx, item, entity)
    changes = []
    if item.cursor
  ...
  def self.match_student(dbx, item, entity)
    changes = []
    if item.cursor
